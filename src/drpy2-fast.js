...
const UpdateInfo = [
    {
        ...
        version: '3.9.53 20251007',
        msg: `
drpy-core-lite.min.js 内置Buffer库
gzip和ungzip改为新so的zlib实现
内置RSA对象加解密效率提升
 ...
 */
function gzip(str) {
    let arr = zlib.gzip(str);
    return Buffer.from(arr, 'utf-8').toString('base64');
}
    ...
    encryptMergedData: function (publicKey, data) {
        ...
        if (dataBuffer.length > segmentLength) {
            ...
            for (let i = 0; i < dataBuffer.length; i += segmentLength) {
                segments.push(dataBuffer.slice(i, i + segmentLength));
            }
            // 逐段加密
            const encryptedSegments = [];
            for (const segment of segments) {
                const encryptedSegment = new Uint8Array(crypto.subtle.encrypt(
                    {
                        name: "RSA-PKCS1-v1_5",
                    },
                    publicKey,
            ...
            // 合并加密后的数据
            const totalLength = encryptedSegments.reduce((acc, segment) => acc + segment.length, 0);
    ...
    decryptMergedData: function (privateKey, mergedData) {
        ...
        if (mergedData.length > segmentLength) {
            ...
            for (let i = 0; i < mergedData.length; i += segmentLength) {
                segments.push(mergedData.slice(i, i + segmentLength));
            }
            // 预分配一个足够大的 Uint8Array
            const totalLength = segments.length * segmentLength;
            let decryptedData = new Uint8Array(totalLength);
            let offset = 0;
            for (const segment of segments) {
                const decryptedSegment = new Uint8Array(crypto.subtle.decrypt(
                    {
                        name: "RSA-PKCS1-v1_5",
                    },
                    privateKey,
                ...
                decryptedData.set(decryptedSegment, offset);
                offset += decryptedSegment.length;
            }
