import{cheerio,模板}from"../dist/drpy-core.min.js";let vercode=typeof pdfl==="function"?"drpy2.1":"drpy2";const VERSION=vercode+" 3.9.52beta2 20250729";const UpdateInfo=[{date:"20250729",title:"drpy更新，所有依赖打包成一个js文件",version:"3.9.52beta2 20250729",msg:`
 1. wasm支持
 ...
 2. rule增加 searchNoPage 属性，可以不定义，如果定义 1 将关闭该源的搜索翻页功能，超过1页直接返回空     
       `}];function getUpdateInfo(){return UpdateInfo.map(_o=>{_o.msg=_o.msg.trim().split("\n").map(_it=>_it.trim()).join("\n");return _o})}function init_test(){console.log("init_test_start");console.log("当前版本号:"+VERSION);console.log("typeof 模板:",typeof 模板);console.log("typeof cheerio:",typeof cheerio);console.log("typeof gbkTool:",typeof gbkTool);console.log("typeof CryptoJS:",typeof CryptoJS);console.log("typeof JSEncrypt:",typeof JSEncrypt);console.log("typeof NODERSA:",typeof NODERSA);console.log("typeof pako:",typeof pako);console.log("typeof JSON5:",typeof JSON5);console.log("typeof JSONPath:",typeof JSONPath);console.log("typeof jinja:",typeof jinja);console.log("typeof WebAssembly:",typeof WebAssembly);console.log("typeof TextEncoder:",typeof TextEncoder);console.log("typeof TextDecoder:",typeof TextDecoder);console.log("typeof WXXH:",typeof WXXH);console.log(gbkTool.encode("你好"));console.log(gbkTool.decode("%C4%E3%BA%C3"));const s='{"method":"GET","timestamp":1745206708456,"path":"/index/fuck","parameters":{"timestamp":["1745206708456"]},"body":""}';const seed=1745206708;const hash=WXXH.h64(s,seed).toString(16);console.log(`WASM:${hash}`);console.log(cheerio.jinja2("渲染一个变量{{hash}}",{hash:hash}));console.log("jsonpath取值测试:",cheerio.jp("$.name",{name:"道长",project:"drpys"}));console.log("本地代理地址:"+getProxyUrl());console.log(RKEY);console.log(JSON.stringify(rule));console.log("init_test_end")}function ocr_demo_test(){let img_base64=``;OcrApi.api=OCR_API;let code=OcrApi.classification(img_base64);log("测试验证码图片的ocr识别结果为:"+code)}function rsa_demo_test(){let t1=(new Date).getTime();let pkcs1_public=`
